...
public final class ZooKeeperUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ZooKeeperUtils.class.getName());

    /**
     * An appropriate default session timeout for Twitter ZooKeeper clusters.
     */
    public static final int DEFAULT_ZK_SESSION_TIMEOUT = 4000;
     ...
     * @throws ZooKeeperClient.ZooKeeperConnectionException if there was a problem accessing the ZK cluster
    ...
    private static void ensurePathInternal(ZooKeeperClient zkClient, List<ACL> acl, String path)
        ...
        if (zkClient.get().exists(path, false) == null) {
            ...
            try {
                ...
                logger.info("Node existed when trying to ensure path {}, somebody beat us to it?", path);
            }
        }
    }
